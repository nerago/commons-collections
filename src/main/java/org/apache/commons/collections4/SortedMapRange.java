...
@SuppressWarnings({"unused", "WeakerAccess", "PublicMethodNotExposedInInterface", "BooleanParameter"})
public final class SortedMapRange<K> implements Serializable {
    ...
    public <V> SortedMap<K, V> applyToMap(final SortedMap<K, V> map) {
        if (fromKey != null && fromInclusive && toKey != null && !toInclusive) {
            ...
            return applyToNavMap((NavigableMap<K, V>) map);
        } else if (isFull()) {
            return map;
        } else if (isEmpty()) {
            return Collections.emptyNavigableMap();
